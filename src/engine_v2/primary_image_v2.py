...
السياسة المتفق عليها مع المالك:
- الصورة الرئيسية (واجهة الصنف في المتجر): {item}_{unit} أو
  {item}_حبة_شدة_كرتون (join_all) — **بلا رقم**.
- بقية الصور: نفس الاسم + ‎-1، ‎-2، ‎-3 حسب ترتيبها.

2.9.12 — اصطلاح المالك الجديد: «الأولى بدون رقم والثانية 1
والثالثة 2»، فصارت البقية تبدأ من ‎-1. وتبقى الوحدة متلازمة
مع `naming_v2.build_name_join_all`: تغيير أحدهما دون الآخر
يُنتج اسمين مختلفين للصورة الواحدة.

تاريخ القرار: في 2.9.9 اعتُمد البدء من ‎-2 لمنع توهّم أن
‎-1 هي الواجهة؛ ثم رجع المالك عنه في 2.9.12 وطلب البدء
من ‎-1 صراحةً، ومعه ترحيل تلقائي للمجلدات القديمة
(`naming_v2.migrate_legacy_dash_names`).
...
def _stem_names(item: str, units: list[str], count: int,
    ...
    # النمط dash الافتراضي (2.9.12): الرئيسية بلا رقم ثم -1/-2…
    # الرقم الظاهر = الرتبة ناقص واحد، فالفهرس i يُكتب كما هو.
    unit = units[0]
    base = f"{item}_{unit}"
