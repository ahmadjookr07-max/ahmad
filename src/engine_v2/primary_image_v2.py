...
- بقية الصور: نفس الاسم + ‎-2، ‎-3، ‎-4 حسب ترتيبها.

2.9.9 — تصحيح الترقيم: كانت البقية تبدأ من ‎-1 فتنتج
`حبه` ثم `حبه-1` ثم `حبه-2`، وهذا خلاف ما أقرّه المالك
نصًّا: «الأولى بلا رقم، الثانية ‎-2، الثالثة ‎-3». صار
الترقيم يبدأ من 2 مطابقًا `naming_v2.build_name_join_all`.
...
def _stem_names(item: str, units: list[str], count: int,
    ...
    # النمط dash الافتراضي: الرئيسية بلا رقم ثم -2/-3…
    # (2.9.9: `i` كان يُستخدم مباشرة فيعطي -1 للثانية؛ الصحيح `i + 1`.)
    unit = units[0]
    base = f"{item}_{unit}"
    return [base if i == 0 else f"{base}-{i + 1}" for i in range(count)]
