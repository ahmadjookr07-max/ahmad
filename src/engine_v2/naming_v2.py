...
def build_name_dash(item: str, seq: int = 1, unit: str = UNIT_SUFFIX_DEFAULT,
    ...
    - الإضافية الأولى (الصورة الثانية): ``-1``
    - الإضافية الثانية (الثالثة): ``-2`` … وهكذا
    - الرقم في **نهاية الاسم دائمًا** بعد الوحدة، بشرطة، ولا شيء بعده.

    أمر المالك النهائي نصًا: «الصورة الأولى (الواجهة): تأخذ الاسم
    الأساسي كما هو بدون ترقيم نهائيًا … الثانية: يضاف لآخرها -1 …
    الثالثة: يضاف لآخرها -2».

    تنبيه لمن يعدّل لاحقًا: في 2.9.9 جُعلت الثانية ``-2`` بحجة أن
    ``base`` و``base-1`` يتداخلان. والحجة باطلة: الرئيسية بلا رقم
    فلا يوجد اسمان متماثلان، والمالك أمر بـ``-1`` مرتين.
    """
    item = sanitize_item(item)
    unit = clean_unit(sanitize_item(str(unit))) if unit else ""
    unit = unit or UNIT_SUFFIX_DEFAULT
...
def parse_name(stem: str) -> ParsedName | None:
    ...
    if m and not m.group("unit").isdigit():
        # قاعدة المالك: الرقم الظاهر هو **رقم الإضافية** لا رتبة
        # الصورة: الرئيسية بلا رقم (seq=1)، و`-1` هي الصورة الثانية
        # (seq=2)، و`-2` هي الثالثة (seq=3). فالرتبة = المعروض + 1
        # ليبقى round-trip متسقًا مع ``build_name_dash`` (تحرسه
        # اختبارات الذهاب والعودة في test_naming_v2).
        shown = int(m.group("seq"))
        return ParsedName(m.group("item"), shown + 1, m.group("unit"))
...
def build_name_join_all(item: str, units: list[str] | tuple[str, ...],
    ...
    - الإضافية الأولى (الصورة الثانية): ``…-1``
    - الإضافية الثانية (الصورة الثالثة): ``…-2`` … وهكذا
    ...
    تاريخ القاعدة (مهم لمن يعدّل لاحقًا فلا يعدّها عطبًا فيعكسها):
    المالك أمر بـ ``-1`` للإضافية الأولى في أول مواصفة وفي أمره
    النهائي. وفي 2.9.9 غُيّرت إلى ``-2`` بحجة «تداخل مع الرئيسية»،
    وهي حجة باطلة: الرئيسية **بلا رقم** فاسمها ``base`` لا يماثل
    ``base-1`` فلا تصادم أصلًا (يحرسه ``test_naming_join_all``).
    """
    item = sanitize_item(item)
    ...
    if seq <= 1:
        return base
    # قاعدة المالك: الرئيسية بلا رقم، والثانية -1، والثالثة -2 …
    return f"{base}-{seq - 1}"
...
class NamingSettings:
    ...
    def _fmt_seq(self, seq: int) -> str:
        """رقم الصورة الإضافية — قاعدة المالك النهائية (2.9.10).

        نص أمر المالك حرفيًا:
        - الصورة الأولى (الواجهة، المعيّنة بالنجمة ★): الاسم
          الأساسي **بلا أي ترقيم نهائيًا**
        - الصورة الثانية (الإضافية الأولى): ``-1``
        - الصورة الثالثة (الإضافية الثانية): ``-2`` …وهكذا

        أي أن الرقم هو **ترتيب الصورة بين الإضافيات** لا ترتيبها
        بين كل صور الصنف: ``seq=2`` ⇒ ``-1`` و``seq=3`` ⇒ ``-2``.

        لا تداخل مع الرئيسية لأنها **بلا رقم أصلًا**، فاسمها
        ``{item}_{units}`` لا يماثل ``{item}_{units}-1`` بحال.

        ``seq_start`` يبقى محترمًا كإزاحة: 1 = قاعدة المالك،
        وقيمة أخرى تُزيح العدّ (إعداد متقدّم).
        """
        n = max(0, seq - 1) + int(self.seq_start) - 1
        if n < 0:
            n = 0
