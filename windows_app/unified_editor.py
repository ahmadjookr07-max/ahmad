...
class UnifiedEditorWidget(V2PhotoEditorDialog):
    ...
    def _build_advanced_panel(self) -> QWidget:
        ...
        # ── بطاقة تقريب وتمركز المنتج داخل المحرر الموسّع ──
        frame_card, fv = card("تقريب وتمركز المنتج")
        frame_hint = QLabel("بعد العزل: قرّب المنتج واضبط موضعه ثم «حفظ واعتماد».")
        frame_hint.setWordWrap(True)
        frame_hint.setStyleSheet("color:#64748b;font-size:11px;")
        fv.addWidget(frame_hint)
        zoom_grid = QGridLayout()
        zoom_grid.addWidget(QLabel("التقريب"), 0, 0)
        self.editor_product_zoom_slider = hslider(100, 130, 106, self._schedule_product_frame)
        self.editor_product_zoom_slider.setSingleStep(1)
        self.editor_product_zoom_slider.setPageStep(1)
        self.editor_product_zoom_slider.setToolTip("خطوة 1% دقيقة — لا يقص المنتج عند الحد الأقصى")
        zoom_grid.addWidget(self.editor_product_zoom_slider, 0, 1)
        self.editor_product_zoom_label = QLabel("106%")
        self.editor_product_zoom_slider.valueChanged.connect(
            lambda v: self.editor_product_zoom_label.setText(f"{v}%"))
        zoom_grid.addWidget(self.editor_product_zoom_label, 0, 2)
        zoom_grid.addWidget(QLabel("أفقي"), 1, 0)
        self.editor_product_offset_x_slider = hslider(-20, 20, 0, self._schedule_product_frame)
        self.editor_product_offset_x_slider.setSingleStep(1)
        self.editor_product_offset_x_slider.setPageStep(1)
        self.editor_product_offset_x_slider.setToolTip("موجب = يمين، سالب = يسار؛ خطوة 1%")
        zoom_grid.addWidget(self.editor_product_offset_x_slider, 1, 1)
        self.editor_product_offset_x_label = QLabel("0%")
        self.editor_product_offset_x_slider.valueChanged.connect(
            lambda v: self.editor_product_offset_x_label.setText(f"{v:+d}%"))
        zoom_grid.addWidget(self.editor_product_offset_x_label, 1, 2)
        zoom_grid.addWidget(QLabel("رأسي"), 2, 0)
        self.editor_product_offset_y_slider = hslider(-20, 20, 0, self._schedule_product_frame)
        self.editor_product_offset_y_slider.setSingleStep(1)
        self.editor_product_offset_y_slider.setPageStep(1)
        self.editor_product_offset_y_slider.setToolTip("موجب = أسفل، سالب = أعلى؛ خطوة 1%")
        zoom_grid.addWidget(self.editor_product_offset_y_slider, 2, 1)
        self.editor_product_offset_y_label = QLabel("0%")
        self.editor_product_offset_y_slider.valueChanged.connect(
            lambda v: self.editor_product_offset_y_label.setText(f"{v:+d}%"))
        zoom_grid.addWidget(self.editor_product_offset_y_label, 2, 2)
        fv.addLayout(zoom_grid)
        frame_buttons = QHBoxLayout()
        frame_apply = QPushButton("توسيط واعتماد الإطار")
        frame_apply.setMinimumHeight(30)
        frame_apply.setToolTip("ينشئ إطار 800×700 من المنتج المعزول لتفعيل التقريب الدقيق")
        frame_apply.clicked.connect(self._smart_frame)
        frame_reset = QPushButton("إعادة للوسط")
        frame_reset.setMinimumHeight(30)
        frame_reset.setToolTip("تقريب تلقائي هادئ 106% وموضع وسط الصورة")
        frame_reset.clicked.connect(self._reset_product_frame_controls)
        frame_buttons.addWidget(frame_apply)
        frame_buttons.addWidget(frame_reset)
        fv.addLayout(frame_buttons)
        fv.addStretch(1)
        row.addWidget(frame_card)
