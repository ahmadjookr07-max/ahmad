...
القواعد المفحوصة:
  ...
  R2  الواجهة ★ بلا رقم، والبقية -1، -2، -3 … (2.9.12)
...
def main() -> int:
    ...
    # ── R2 الترقيم والواجهة ────────────────────────────────────────
    say("\n[R2] الواجهة ★ بلا رقم ثم -1، -2، -3 (2.9.12)")
    u = ["حبه", "باكت"]
    ...
    check("التسلسل -1 ثم -2 ثم -3",
          ...
          str(seq[1:]))
    check("الواجهة وحدها بلا رقم ولا تكرار",
          sum(1 for s in seq if "-" not in s) == 1
    ...
    # 2.9.10 — أمر المالك: دمج الوحدات **خيار** في الواجهة مُلغى
    # افتراضيًا. فأُزيلت الترقية القسرية التي كانت تُعيد
    # تفعيل الدمج كلما غاب علم «الاختيار الصريح»، لأنها
    # تُبطل إلغاء المالك للخانة من حيث لا يرى.
    check("ملف قديم بلا علم صريح يُحترم كما هو (2.9.10)",
          N.NamingSettings.from_dict(
    ...
    try:
        ...
        # 2.9.10: الخانة مُلغاة افتراضيًا → وحدة واحدة (default_unit)،
        # وهو نفس ما تكتبه الواجهة حين يُلغي المالك الخانة.
        check("الافتراضي = وحدة واحدة لا دمج (2.9.10)",
              s0 is not None
    ...
    # 2.9.12: `-1` صار الاسم الرسمي للصورة الثانية، فالقراءة
    # تفهمه أصالةً لا تسامحًا.
    check("يفهم المرقّم -1 (الصورة الثانية)",
