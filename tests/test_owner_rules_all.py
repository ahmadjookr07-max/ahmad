...
القواعد المفحوصة:
  ...
  R2  الواجهة ★ بلا رقم، والإضافيات -1، -2 … (2.9.10: نص المالك)
...
def main() -> int:
    ...
    # ── R2 الترقيم والواجهة ────────────────────────────────────────
    say("\n[R2] الواجهة ★ بلا رقم ثم -1، -2 (2.9.10)")
    u = ["حبه", "باكت"]
    seq = [N.build_name_join_all("777", u, i, total=4) for i in (1, 2, 3, 4)]
    check("الواجهة بلا رقم", seq[0] == "777_حبه_باكت", seq[0])
    check("التسلسل -1 ثم -2 ثم -3",
          seq[1:] == ["777_حبه_باكت-1", "777_حبه_باكت-2", "777_حبه_باكت-3"],
          str(seq[1:]))
    check("لا يوجد رقم -0 ولا اسم مكرر (لا طمس)",
          not any(s.endswith("-0") for s in seq)
          and len(set(seq)) == len(seq), str(seq))
    ...
    # 2.9.10 — {seq} يتبع قاعدة المالك: الرقم ترتيب الصورة بين
    # الإضافيات، فالصورة الثانية (أول إضافية) = 1.
    check("القالب المخصص يُرسم فعلًا", got == "123_628110-1", got)
    ...
    # 2.9.10 — أُلغيت ترقية 2.9.6 القسرية: السياسة المحفوظة تُحترم
    # كما هي، ولا يغيرها إلا المالك من الواجهة — لأن الترقية
    # كانت تُعيد الدمج من تلقاء نفسه فلا يصمد إلغاء الخانة.
    check("ملف قديم يُحترم كما هو (لا ترقية قسرية)",
          N.NamingSettings.from_dict(
    ...
    try:
        ...
        # 2.9.10 — الخانة مُلغاة افتراضيًا بأمر المالك، فالافتراضي
        # الصريح هو الوحدة الواحدة (default_unit)، والدمج باختياره.
        check("الافتراضي الصريح = وحدة واحدة (الخانة مُلغاة)",
              s0 is not None
              and getattr(s0, "unit_policy", "") == N.UNIT_POLICY_DEFAULT,
