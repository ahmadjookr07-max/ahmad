...
# 3) قاعدة الترقيم (2.9.12): الرئيسية بلا رقم، ثم -1 ثم -2.
#    أمر المالك نصًا: «الأولى بدون رقم والثانية 1 والثالثة 2».
#    فالاسم الموروث `X_2_حبه` (الصورة الثانية بالنمط القديم)
#    يُترجم إلى `X_حبه-1` وهو الصحيح، لا إلى `-2`.
main_name, _ = ref._fixed_name("10018435_حبه")
check("الرئيسية بلا رقم",
      not main_name.rsplit("-", 1)[-1].isdigit()
      and "_1_" not in main_name, main_name)

second, _ = ref._fixed_name("10018435_2_حبه")
check("الصورة الثانية تأخذ -1 وتبقى متميزة عن الرئيسية",
      second.endswith("-1") and second != main_name, second)
