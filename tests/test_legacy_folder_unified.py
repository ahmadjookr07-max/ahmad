...
- قاعدة التسمية (2.9.9): الواجهة `رقم_الوحدة` بلا رقم،
  ثم `-2`، `-3`، `-4` — لا `-1` إطلاقًا (كان يتداخل مع الواجهة).
...
def run() -> None:
    ...
    try:
        ...
        got = stems(folder)
        check(got == ["10000015_حبه", "10000015_حبه-2", "10000015_حبه-3"],
        ...
        # 2.9.9 — قاعدة المالك: الواجهة بلا رقم، والبقية
        # تحمل ترتيبها الحقيقي -2 .. -17 (لا -1 إطلاقًا، لأنه
        # كان يتداخل مع الواجهة في نفس المجلد).
        seq = sorted(s for s in stems(folder) if "-" in s)
        expect = [f"777_حبه-{i}" for i in range(2, 18)]
        check(sorted(seq) == sorted(expect),
              "تسلسل 16 صورة متصل بلا ثغرة ولا تصادم")
        check(not any(s.endswith("-1") for s in stems(folder)),
              "لا يوجد -1 المحظور (تداخل مع الواجهة)")
