...
- قاعدة التسمية (2.9.10 — نص المالك): الواجهة `رقم_الوحدة`
  بلا رقم، ثم `-1`، `-2`، `-3` — الرقم ترتيب الصورة بين
  الإضافيات، ولا تداخل مع الواجهة لأنها بلا رقم أصلًا.
...
def run() -> None:
    ...
    try:
        ...
        # 2.9.10 — قاعدة المالك: الواجهة بلا رقم، والإضافيات
        # تُرقّم بترتيبها بينها لا بين كل الصور: -1 .. -16 (
        # سبع عشرة صورة)، ولا تداخل لأن الواجهة بلا رقم أصلًا.
        ...
        check(not any(s.endswith("-0") for s in stems(folder)),
              "لا يوجد رقم -0")
        check(len(set(stems(folder))) == len(stems(folder)),
              "لا اسم مكرر (لا طمس لأي صورة)")
