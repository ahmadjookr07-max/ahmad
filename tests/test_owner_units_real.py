...
def main() -> int:
    ...
    # ── قاعدة الترقيم: الواجهة بلا رقم ثم -1 ──────────────────────
    # 2.9.12 — قرار المالك: الرقم الظاهر يبدأ من 1 لا من 2.
    # الأولى بلا رقم، الثانية -1، الثالثة -2 — فلا تبقى فجوة
    # بين الواجهة وأول صورة مرقّمة كما كان الحال قبل 2.9.12.
    say("\n[3] قاعدة المالك للصور المتعددة (★ الواجهة بلا رقم ثم -1، -2)")
    ...
    check("الثانية تأخذ -1 بعد كل الوحدات",
          n2 == "10008272_باكت_حبه_كرتون_كرتون1-1", n2)
    check("الثالثة تأخذ -2", n3 == "10008272_باكت_حبه_كرتون_كرتون1-2", n3)
    check("الواجهة وحدها بلا رقم والترقيم متصل من 1",
          not n1.endswith("-1") and not n1.endswith("-2")
          and n2.endswith("-1") and n3.endswith("-2"),
          f"{n1} | {n2} | {n3}")
    ...
    say("\n[6] داخل كل صنف: واجهة واحدة بلا رقم والبقية -1، -2 متتالية")
    ...
    for item, stems in by_item.items():
        ...
        if len(bases) != 1:
            ...
            # 2.9.12 — الرئيسية بلا رقم، وأول لاحقة مرقّمة -1
            # فيرى المالك 1، 2، 3 بلا قفزة غير مفهومة.
            bad_seq.append(f"{item}: تسلسل {nums}")
    check("كل صنف له واجهة واحدة بلا رقم وترقيم متصل من 1",
